#!/usr/bin/env python3
"""L1 1,500-page RC batch validator — read-only, stdlib only (Sprint 6M-E).

Validates quarantined non-public 1,500-page RC HTML under site/_sample/ only.
...
def main() -> int:
    print("=" * 60)
    print("Bisulfid 1,500-Page RC Batch Validator — Layer 1")
    ...
    if manifest:
        ...
        if rendered != RC_1500_EXACT:
            all_errors.append(f"rendered count {rendered} != required {RC_1500_EXACT}")
        if manifest.get("batch_id") not in ("rc_1500", "rc_batch_1500"):
    ...
    if len(sample_html) != RC_1500_EXACT:
        all_errors.append(f"HTML file count {len(sample_html)} != {RC_1500_EXACT}")
