#!/usr/bin/env python3
"""L1 release candidate batch validator — read-only, stdlib only (Sprint 6M-D).

Validates quarantined non-public RC Batch 01 HTML under site/_sample/ only.
...
def validate_rc_file(path: Path, manifest_page: dict | None) -> tuple[list[str], list[str]]:
    ...
    if "data-rc-batch" not in lower and "release candidate" not in lower:
        warnings.append(f"{path.name}: RC batch marker could be clearer")
...
def main() -> int:
    print("=" * 60)
    print("Bisulfid Release Candidate Batch Validator — Layer 1")
    ...
    for path in sample_html:
        checked += 1
        if checked <= 5 or checked > len(sample_html) - 2:
            print(f"  Checked: {path.relative_to(ROOT)}")
        elif checked == 6:
            print(f"  ... ({len(sample_html) - 7} more files) ...")
        errs, warns = validate_rc_file(path, manifest_by_output.get(path.name))
    ...
    print(f"VALIDATION SUMMARY: PASS ({len(sample_html)} RC batch file(s))")
    print("=" * 60)
    return 0
