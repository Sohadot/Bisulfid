#!/usr/bin/env python3
"""Sprint 6M-G — governed corpus expansion toward 14,000 public launch foundation pages.
...
INVENTORY_OUT = ROOT / "main/data/COHORT_05_14000_EXPANSION_INVENTORY.json"
MANIFEST_OUT = ROOT / "main/data/COHORT_05_14000_EXPANSION_MANIFEST.json"

COHORT_ID = "COHORT_05_14000_LAUNCH_FOUNDATION"
ENGINE = "generate_14000_corpus_expansion_v1"
TARGET_NEW_ROUTES = 6500
EN_ENTITY_COUNT = 806
SINGLE_ROUTE_COUNT = 52
...
def build_en_entity_catalog_14000() -> list[tuple[str, str]]:
    ...
    for base in METAL_SULFIDE_BASES:
        add(f"c05_metal_{base}", "metal_sulfide_language")
        if len(catalog) >= 161:
            break

    for root, suffix in itertools.islice(itertools.product(ORGANO_ROOTS, ORGANO_SUFFIXES), 161):
        add(f"c05_organo_{root}_{suffix}", "organosulfur_language")
        if len(catalog) >= 322:
            break

    for base in INORGANIC_BASES:
        add(f"c05_inorg_{base}", "inorganic_sulfur_language")
        if len(catalog) >= 483:
            break

    for base in INDUSTRIAL_BASES:
        add(f"c05_ind_{base}", "industrial_language")
        if len(catalog) >= 644:
            break

    for base in MINERAL_BASES:
        add(f"c05_min_{base}", "mineral_geology_language")
        if len(catalog) >= 805:
            break

    for base in DISAMBIG_BASES:
        add(f"c05_dis_{base}", "disambiguation_prep")
...
def build_single_routes() -> list[tuple[str, str, str, str, str, str]]:
    """Return (lang, route_id, path, h1, page_family, template) — exactly 52 singles."""
    singles: list[tuple[str, str, str, str, str, str]] = []

    for base in DE_TERM_BASES[:20]:
        slug = path_slug(base)
        rid = f"cohort05_de_{base}"
        singles.append((
            "de", rid, f"/de/terminology/cohort05/{slug}/",
            f"{display_term(base)} — DE Terminologie", "de_terminology", "term_page.html",
        ))

    for base in DISAMBIG_BASES[:10]:
        slug = path_slug(base)
        rid = f"cohort05_en_dis_{base}"
        singles.append((
            "en", rid, f"/reference/cohort05/{slug}/",
            f"{display_term(base)} — Disambiguation", "disambiguation", "reference_page.html",
        ))

    gov_topics = (
        ("en_gov_14000_launch_foundation", "/foundation/14000-launch-foundation/", "14,000-page launch foundation"),
        ("en_gov_public_visibility_gate", "/foundation/public-visibility-gate/", "Public visibility gate explainer"),
        ("en_gov_indexation_gate_closed", "/foundation/indexation-gate-closed/", "Indexation gate closed explainer"),
        ("en_gov_sitemap_gate_closed", "/foundation/sitemap-gate-closed/", "Sitemap gate closed explainer"),
        ("en_gov_navigation_gate_closed", "/foundation/navigation-gate-closed/", "Navigation gate closed explainer"),
        ("en_gov_cohort05_spine", "/reference/cohort-05-spine/", "Cohort 05 terminology spine"),
        ("en_gov_source_claim_14000", "/reference/source-claim-14000/", "14,000 source/claim boundary"),
        ("en_gov_multilingual_14000", "/reference/multilingual-14000/", "Multilingual 14,000 corpus map"),
        ("de_gov_14000_launch_foundation", "/de/foundation/14000-launch-foundation/", "14.000-Seiten-Launch-Foundation"),
        ("de_gov_public_visibility_gate", "/de/foundation/public-visibility-gate/", "Sichtbarkeits-Gate"),
        ("de_gov_source_claim_14000", "/de/reference/source-claim-14000/", "14.000 Quellen/Claim-Grenze"),
        ("de_gov_cohort05_spine", "/de/reference/cohort-05-spine/", "Cohort-05-Terminologie-Spine"),
    )
    for rid_suffix, path, h1 in gov_topics:
        lang = "de" if path.startswith("/de/") else "en"
        rid = f"cohort05_{rid_suffix}"
        singles.append((lang, rid, path, h1, "reference_governance", "reference_page.html"))

    gateway_topics = (
        ("en_index_cohort05_spine", "/reference/cohort-05-index/", "Cohort 05 index hub"),
        ("en_index_14000_families", "/reference/14000-families-index/", "14,000 route families index"),
        ("en_index_en_terminology_14000", "/reference/en-terminology-14000/", "EN terminology 14,000 hub"),
        ("en_index_de_terminology_14000", "/reference/de-terminology-14000/", "DE terminology 14,000 hub"),
        ("en_index_source_required_14000", "/reference/source-required-14000/", "Source-required 14,000 hub"),
        ("en_index_public_launch_foundation", "/reference/public-launch-foundation/", "Public launch foundation hub"),
        ("de_index_cohort05_spine", "/de/reference/cohort-05-index/", "Cohort-05-Index-Hub"),
        ("de_index_14000_families", "/de/reference/14000-families-index/", "14.000-Routenfamilien-Index"),
        ("de_index_source_required_14000", "/de/reference/source-required-14000/", "Quellenpflicht-14.000-Hub"),
        ("de_index_public_launch_foundation", "/de/reference/public-launch-foundation/", "Public-Launch-Foundation-Hub"),
    )
...
**Draft status:** This page is a **non-public** COHORT_05 pipeline expansion draft. It is **not published**, **not indexable**, **not in the sitemap**, **not in navigation**, and **not publication-ready**. This is not a public launch page. **No claim is approved** for publication from this draft alone.
...
- `route_id`: `en_index_cohort05_spine` (planning placeholder — not a live link)

*Non-public COHORT_05 draft — {COHORT_ID} — engine v1.*
"""
...
def build_cohort05_rows(existing_ids: set[str], existing_paths: set[str]) -> list[dict]:
    rows: list[dict] = []
    seq = 1

    for entity, category in build_en_entity_catalog_14000():
        for vkey, aud_key, ref_key, pt, aud, ref, template in ENTITY_VARIANTS:
            rid = f"cohort05_en_{entity}_{vkey}_{aud_key}_{ref_key}"
            if rid in existing_ids:
                continue
            path = f"/en/terminology/cohort05/{path_slug(entity)}/{vkey}/{aud_key}/{ref_key}/"
            if path in existing_paths:
                raise ValueError(f"path collision: {path}")
            rows.append({
                "inventory_row_id": f"INV-C05-{seq:05d}",
                "route_id": rid,
                "language": "en",
                "route_path": path,
                ...
                "template": template,
                "content_subdir": "cohort-05-expansion",
            })
            seq += 1
    ...
    for lang, rid, path, h1, page_family, template in build_single_routes():
        ...
        rows.append({
            ...
            "content_subdir": "cohort-05-expansion/de" if lang == "de" else "cohort-05-expansion",
        })
        seq += 1
...
def route_record_from_row(row: dict) -> dict:
    ...
    return {
        ...
        "risk_level": "medium",
        "notes": (
            f"Sprint 6M-G. COHORT_05 14,000-page launch foundation expansion ({row.get('page_type_id', 'governed')}). "
...
def run(write: bool) -> int:
    print("=== Sprint 6M-G Corpus Expansion v1 ===")
    ...
    new_rows = build_cohort05_rows(existing_ids, existing_paths)
    ...
    if write:
        ...
        links_data["link_groups"].append({
            "from_route_id": "en_index_cohort05_spine",
            "status": "planned",
    ...
    print(f"New COHORT_05 routes: {len(new_routes)}")
